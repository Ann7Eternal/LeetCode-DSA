// Last updated: 2/14/2026, 1:41:33 PM
class Solution {
public:
    string numberToWords(int num) 
    {
        ...
        string result = WordsHelper(num%1000);
        num = num/1000;

        for(int i=0;i<3;++i)
        {
            if(num>0 && num%1000>0)
            result = WordsHelper(num%1000) + bigString[i] + " " + result;
            num = num/1000;
        }
        return result.empty() ? result : result.substr(0, result.size()-1);
    }

    string WordsHelper(int num)
    {
        ...
        string result = "";
        if(num>99)
        result = result + (digitString[num/100] + " Hundred ");
        num %= 100;
        if (num < 20 && num > 9)
